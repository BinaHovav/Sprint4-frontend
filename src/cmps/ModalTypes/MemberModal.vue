  ...
  methods: {
    toggleTaskMember(member) {
      if (this.info.task.members.includes(member._id)) {
        ...
        if (loggedinUser._id === member._id) action = { type: 'left', txt: `${this.info.task.title}`, componentId: '', movedCmp: '', movedUser: '' }
        ...
        if (loggedinUser._id === member._id) action = { type: 'joined', txt: `${this.info.task.title}`, componentId: '', movedCmp: '', movedUser: '' }
